...
use aik099\QATools\HtmlElements\Elements\SelectOption;
use Mockery as m;
...
class SelectOptionTest extends TypifiedElementTest
{
	...
	protected function setUp()
	{
		if ( is_null($this->elementClass) ) {
			$this->elementClass = '\\aik099\\QATools\\HtmlElements\\Elements\\SelectOption';
		}

		$this->select = m::mock('\\aik099\\QATools\\HtmlElements\\Elements\\Select');
	...
	public function testDeselect($checked, $driver_class)
	{
		...
		if ( $driver_class == self::SELENIUM_DRIVER_CLASS ) {
			...
			if ( !$checked ) {
				$this->setExpectedException('\\aik099\\QATools\\HtmlElements\\Exceptions\\TypifiedElementException');
			}
